...
class SimpleLinRegModel:
    ...
    def compute_gradient(self, dataset):
        ...
        y = dataset[:,1]
        n = len(dataset)
        dL_dw = (1/n) * np.sum(((self.slope * x + self.bias) - y) * x)
        dL_db = (1/n) * np.sum(self.slope * x + self.bias)

        grad = np.array([dL_dw, dL_db])
        # --------------------------------#
...
class FLServer:
    ...
    def on_parameters_received(self, simulator, param_list):
        ...
        # print("# of Clients:", len(param_list))        
        n_k = len(param_list)
            
        for param in param_list:
            self.params.slope = self.params.slope + param.slope
            self.params.bias = self.params.bias + param.bias

        self.params.slope = self.params.slope / n_k
        self.params.bias = self.params.bias / n_k
        # --------------------------------#
...
class FLClient:
    ...
    def load_dataset(self):
        ...
        df = pd.read_csv(f'dataset_client{self.rank}.csv')
        dataset = df.to_numpy() 
        # dataset = np.array(df)
    ...
    def on_parameters_received(self, simulator, server_params):
        ...
        for _ in range(self.num_local_updates):
            grad = server_params.compute_gradient(self.dataset)
            self.params.slope = self.params.slope - self.local_step_size * grad[0]
            self.params.bias = self.params.bias - self.local_step_size * grad[1]
        # --------------------------------#
